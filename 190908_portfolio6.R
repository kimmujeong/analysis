...
best.iter = lgb.model.cv$best_iter #예제 525, 실제돌려보면 300~310부근나옴, 391(set.seed(257))
best.iter=525
...
lgb.model.cv2 = lgb.cv(params = lgb.grid, data = lgb.train, learning_rate = 0.02, num_leaves = 25,
                      num_threads = 2 , nrounds = 7000, early_stopping_rounds = 50, #nrounds:7000
                      eval_freq = 20, eval = lgb.normalizedgini,
                      nfold = 5, stratified = TRUE)
best.iter = lgb.model.cv2$best_iter #435
...
test2$target<-result
write.csv(test2[,c("id","target")], file="/home/kmj/save_csv/save_lightgbm_select_var_best_iter_435.csv", row.names = FALSE)
