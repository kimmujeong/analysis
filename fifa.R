...
test<-read.csv("C:\\Users\\thgus\\Desktop\\fifa.csv",header=T,encoding = "UTF-8")
...
#전체 오버롤 분포도
ggplot(test,aes(x=Overall))+geom_histogram(color = "white", fill = "darkgrey")+
  ggtitle("오버롤 전체 분포도", subtitle = "서브타이틀")

###############################################################################

#나이대별 오버롤 평균
overall_age_mean<-aggregate(Overall~Age,test,mean)
overall_age_mean
...
#국가별 오버롤 평균
nation_overall_mean<-aggregate(Overall~Nationality,test,mean)
...
#팀별오버롤 상위20개[1]
team_overall<-aggregate(Overall~Club,test,mean)
...
#탑20 확인
head(subset(test2,select = c(Name,new_wage)),n=20)
...
#그냥
ggplot(test,aes(x=Overall))+geom_histogram(binwidth = 5,fill="blue",color="black")+facet_grid(Age~.)
ggplot(test,aes(x=Overall))+geom_histogram(binwidth = 5,fill="blue",color="black")+facet_grid(.~Age)
